...
* `GetDatabaseURL` doesn't URL-encode the password, which could fail if the password contains special characters.
* `Validate` only checks for presence of required fields, not their format (e.g., valid base64, valid URL, valid port numbers).
